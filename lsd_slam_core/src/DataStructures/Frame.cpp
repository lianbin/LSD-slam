...
void Frame::buildGradients(int level)
{
	...
	const float* img_pt = data.image[level] + width;//从第二行开始
	const float* img_pt_max = data.image[level] + width*(height-1);//倒数第二行结束
	...
	for(; img_pt < img_pt_max; img_pt++, gradxyii_pt++)
	{
		...
		*((float*)gradxyii_pt) = 0.5f*(val_p1 - val_m1);//左右中心差分
		*(((float*)gradxyii_pt)+1) = 0.5f*(*(img_pt+width) - *(img_pt-width));//上下中心差分
		*(((float*)gradxyii_pt)+2) = val_00;//像素值

		val_m1 = val_00;//更新左侧像素值
		val_00 = val_p1;//更新当前的像素值
	}

	data.gradientsValid[level] = true;//设置为合法的梯度level
}
...
void Frame::buildMaxGradients(int level)
{
	...
	int width = data.width[level];
	int height = data.height[level];
	if (data.maxGradients[level] == 0)//分配内存
    ...
    //以像素点为中心上中下，左中右，十字架形状的区域，上中下三个像素中取最大值作为中心像素的幅值
	...
	if(level==0)
		this->numMappablePixels = numMappablePixels;//梯度幅值达到要求的像素个数

	FrameMemory::getInstance().returnBuffer(maxGradTemp);

	data.maxGradientsValid[level] = true;//最大梯度合法标识
}
